...
import { useComplaint, useSetComplaintStatus } from "../hooks/useComplaints"

export default function ComplaintDetailPage() {
    const { id } = useParams()
    const { data: item, isFetching, error, refetch } = useComplaint(id)
    ...
    useEffect(() => {
        if (error) {
            toast({ title: "Not found or no access", description: error.message, status: "error" })
            navigate("/complaints")
        }
    }, [error])
    ...
    const onSetStatus = (status) => {
        setStatus.mutate({ id, status }, {
            onSuccess: () => { toast({ title: "Status updated", status: "success" }); refetch() },
            onError: (err) => toast({ title: "Failed to update status", description: err.message, status: "error" }),
        })
    }
